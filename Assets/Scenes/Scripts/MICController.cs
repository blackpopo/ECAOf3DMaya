...
public class MICController : MonoBehaviour
{
    ...
    const int FFTSAMPLES = 2 << 7; //256bitのサンプルをFFTでは選んでとる。
    ...
    const float SST_SPEAKING_TIME = 3.0f;
    ...
    { 
        CalculateVowel();
        if ((MIN_FREQ < frequency && frequency < MAX_FREQ && MIN_VOLUME < volume && !voiceController.isAudioPlaying()) || forceRecording)　//しゃべり始めの時間
        {
            if (forceRecording)
            {
            ...
            {
                inputImage.color = Color.magenta;
            }
            isSpeaking = true;
        ...
        // else if ((isSpeaking && volume > MIN_VOLUME)  || forceRecording)
        // {
        //     //子音字をしゃべっていると判定
        //     bufferTimer = 0.0f;
        //     speakingTimer += Time.deltaTime;
        // }
        else if ((isSpeaking && bufferTimer < BUFFER_TIME && !voiceController.isAudioPlaying()) || forceRecording) // 余韻の時間
        {
        ...
        else if (!forceRecording)
        {
            ...
            if (isRecording)
            {
                ...
                if (speakingTimer > SST_SPEAKING_TIME)　//もし、一定時間以上話していたら、音声を認識する。
                {
                    ...
                    sentence =  await stt.stt(SSTFileName);
                    // sentence = "こんにちは。";
                    StartCoroutine(SetSenteceNull(0.8f));
                }
            }
   ...
   private void WavBufferWrite(FileStream fileStream, List<float> dataList)
    ...
    IEnumerator SetSenteceNull(float timer)
    {
    ...
    {
        var scale = 8000.0f;
        gain = 1000.0f + scale * gainSlider.value;
    } 

    private void setGainSlider()
    {
        gainSlider.value = (gain - 1000.0f) / 8000.0f;
    }

    public void forceRecordingMode()
    {
